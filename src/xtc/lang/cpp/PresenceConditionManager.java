...
class PresenceConditionManager {
  ...
  public BDD createConfiguration(boolean defaultSetting,
    ...
    for (int i = 0; i < vars.indices.size(); i++) {
      ...
      if (null != nonbooleans) {
        for (String key : nonbooleans.keySet()) {
          if (varString.contains(key) &&
              ! varString.equals("(defined " + key + ")")) {
            varString = varString.replaceAll(key, nonbooleans.get(key));
            containsNonboolean = true;
          }
        }
      }
      ...
      if (containsNonboolean) {
        ...
        if (result.isZero()) {
          ...
          System.err.println("unresolved boolean variable");
          System.err.println(varString);
          // System.exit(1);
        }

        if (true == defaultSetting) {
          setting = ! setting;
        }
      } else if (null != exceptions && exceptions.contains(varString)) {
        setting = ! setting;
      }
   ...
   */
  public BDD allYes() {
    return createConfiguration(true, null, null, null);
  }
   ...
   */
  public BDD allNo() {
    return createConfiguration(false, null, null, null);
  }
