#!/bin/sh

################################################################################
#
# A demo / run script.
# Step 1. Convert CQL query to SRA
# Step 2. Convert SRA query to Brooklet
# Step 3. Convert Brooklet to System S
#
################################################################################

TEST=origin-fast
SZ_FILE=../../../../fonda/babble_testsuite/sawzall/$TEST.sz
BKLT_FILE=$TEST.bk
FAST_FILE=./fast.bk
IMPL_FILE=./FunctionEnv/operators.ml
TMPLT=./watson/templates


if [ ! -f ./hosts.txt ];
then
    echo "ERROR: missing hosts.txt file."
    exit 1
fi


if [ ! -f ./parallel_and_fuse.opt ];
then
    echo "ERROR: missing parallel_and_fuse.opt file."
    exit 1
fi


java xtc.lang.babble.optimizer.Optimizer -spec parallel_and_fuse.opt -output $FAST_FILE -implFile $IMPL_FILE $BKLT_FILE

if [ -d ./MyApp ];
then
    echo "\n\nRemoving existing MyApp directory\n\n"
    rm -rf ./MyApp
fi
java xtc.lang.babble.watson.Watson -logLevel error -hostFile ./hosts.txt -user $USER -templatePath $TMPLT -toSystemS $FAST_FILE

cp fast-operator.ml MyApp/src/operators.ml 



