...
void UDOP_f1::initializeOperator()
{
  numTuples = 0;
...
void UDOP_f1::finalizeOperator()
{
  shmdt((void*)mutexes);
}

void UDOP_f1::processCmdArgs(const string & args)
{
  if (args.empty()) {
    return;
  }
...
void UDOP_f1::process()
{
  /* only defined for Source operators */
}

void UDOP_f1::process(uint32_t port, const SBuffer & buf)
{
  stringstream msg;
  ...
  func = *(caml_named_value("ocaml_wrap_f1"));
...
void UDOP_f1::shutdown() 
{
  CAMLparam0();
  CAMLlocal2(func, ptr);
  ptr = Val_long(this);
  func = *(caml_named_value("f1_shutdown"));
